...
async function handleAnalysisRequest(e) {
    ...
    if (analysisType === 'histogram') {
        ...
        if (!categoryColumn || !valueColumn) {
            showError('Please select both category and value columns for the bar graph');
            return;
        }
...
function displayResults(analysisType, data) {
    ...
    if (analysisType === 'summary') {
        ...
        // Add message if columns were limited
        if (data.limited_columns && data.message) {
            correlationHTML += `<div class="correlation-message">${data.message}</div>`;
        }
        ...
        // Display bar graph with cache-busting parameter
        if (data.plot_url) {
            const categoryColumn = document.getElementById('bar-category-column').value;
            const valueColumn = document.getElementById('bar-value-column').value;
            barPlot.innerHTML = `<img src="${data.plot_url}?t=${new Date().getTime()}" alt="Bar Graph of ${valueColumn} by ${categoryColumn}">`;
        }
...
function handleDownload(e) {
    ...
    if (downloadType === 'histogram' || downloadType === 'scatter' || downloadType === 'correlation' || downloadType === 'bar') {
...
function downloadPlotImage(plotType, button, originalText) {
    ...
    if (plotType === 'histogram') {
        ...
        const categoryColumn = document.getElementById('bar-category-column').value;
        const valueColumn = document.getElementById('bar-value-column').value;
        filename = `bar_${valueColumn}_by_${categoryColumn}_${formatDate()}.png`;
